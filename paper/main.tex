...
\item[Apply HIL Callback] Performs an UPDATE on the \texttt{hil\_reviews} row, setting \texttt{approval\_status}, \texttt{approval\_decided\_at}, and \texttt{approval\_actor} (the Telegram user). The update is idempotent: a second tap has no further effect.
...
\input{figures/posterior_table.tex}
\end{table}
...
\begin{figure}[!t]
\centering
\input{figures/calibration_data.tex}
